...
function unrecognizedFactResidue(
  ...
  return normalizedFactSupportText(
    residue.replace(/(?:患者|本次|当前|目前|症见|表现为|伴有|伴|并有|以及|及|和|与|、)/g, "")
      // 时间/频度/程度副词与症状本体在病历里常分属两个子句（「下腹部发热」…「多在下午3时后
      // 出现」），模型引用时几乎必然重组成「下午3时后下腹部发热」。重组不产生新的临床断言，
      // 把这些修饰词算进「未识别残留」会让发热类病历稳定触发 *_literal、修复同码不收敛
      //（实测网络医案 10/31「内伤发热」连中 4 轮同码、整页作废）。症状本体仍须逐项接地。
      .replace(/(?:凌晨|清晨|晨起|上午|中午|下午|午后|傍晚|夜间|夜晚|入夜|睡前|饭前|饭后|餐后|空腹|活动后|劳累后|受凉后|生气后|情绪激动后|经期|经前|经后|多在|每于|每逢|每天|每日|每次|时后|时许|左右|约|反复|阵发性|阵发|间断|持续|长期|偶有|偶发|时有|时作|常|易|加重|减轻|明显|轻度|中度|重度|不定时|[0-9０-９一二三四五六七八九十]+[时点日天周月年度℃]+)/g, ""),
  );
}
...
const TCM_THERAPY_CONCEPTS: ReadonlyArray<[TcmTherapyConcept, RegExp]> = [
  ...
  // 「退热」被刻意排除：它是疗效目标词，不是方法词——辛温解表（桂枝汤解肌退热）、甘温除热
  // 同样以退热为目标。目标词进方法表的后果是 required 被塞进 heat_clear，方中每一味温药
  // 都触发方向对立否决（实测网络医案 42：调和营卫，解肌退热 → 桂枝被判 yang_warm 对立 → 0 味）。
  ["heat_clear", /清热|泻火|凉血|解毒|辛凉|清[泄泻宣透利](?!湿)|清(?:肺|肝|心|胃|营|暑|肠|胆|骨|金|气|血)|泄热(?!因)|除烦|降火|潜降虚火|泻[肺肝心胃肠胆火]|清化(?![^，。；;]{0,2}痰)|清解|气血双清|泻南补北|佐金平木|寒因寒用/],
  ...
  // 「胃」与「暖胃」是 2026-08-16 补入。此前 温(?:中|肾|里|肺|经|化|补|通|养) 里独缺「胃」、
  // 暖宫有而暖胃无，于是**温胃解析为空**——藏菖蒲（药典「温胃，消炎止痛」，中医师终审取
  // 「温胃，止痛」）因此进不了功效表。
  // 依据：温里回阳法 = 温中祛寒 + 回阳救逆；温中祛寒「是治疗中焦虚寒的方法」；
  // 「若胃中虚寒，浊阴上逆，又当温中祛寒与降逆法同用」（中医百科·温里回阳法）。
  // 温胃法（温胃散寒）适用于胃寒证，常用吴茱萸汤、良附丸——即温中祛寒施于胃，同族。
  // 爆炸半径实测（遍历 695 味）：含「温胃」者 1 味（厚朴，已由其他功效词解析出 yang_warm，
  // 不依赖本词）、含「暖胃」者 0 味 ⇒ **收词后新增匹配 0 味**，实际影响面仅藏菖蒲一味。
  ["yang_warm", /温阳|扶阳|回阳|散寒|辛温|温(?:中|胃|肾|里|肺|经|化|补|通|养)|补阳|[温补][补益]?[肾脾心]{1,2}阳|壮阳|益火|引火归[原元]|辛甘化阳|温脏|暖宫|暖胃|温下|甘温除(?:大)?热|热因热用/],
  ...
  ["exterior_release", /解表|祛风|疏风|疏散风邪|疏风散邪|发散风寒|发散风热|疏散风热|凉散风热|疏风散热|散风|解肌|透疹|透表|发汗|疏解|辛散|求汗|轻宣|宣散|治风法|解肌/],
  ...
  ["purge", /通便|泻下|攻下|逐水|通腑|急下|消导法?|杀虫|驱蛔|安蛔|以毒攻毒|通因通用/],
  ...
  ["astringe", /收涩|敛汗|固涩|固精|止带|固经|固冲|固摄|止崩|涩肠|涩精|固脱|固护|固脬|缩尿|止遗|敛[肺阴疮]|收敛|塞因塞用/],
 ...
 *
 * ⚠ 这个集合**刻意小于** m04-repair-policy 的 m04TherapyIssueQualityAnnotation，两者不是同集，
 * 也不该是。原因在两个调用点（下方 3505 / 3749）的写法里：它们都是
 * `waiveTherapyCoverageAnnotated && isWaivable(coverageIssue)`，而 coverageIssue 正是在
 * **同一个 waive 口径**下算出来的——waive=true 时 unsupportedHighImpactHerbIssue 会把
 * 「词表未成立」那一半清空（vocabUnsupported = []），只剩「方向对立」。
 * 实测（M03 锁定「清热泻火」）：
 *     附子 waive=true → transparent_therapy_herb_1_unsupported_high_impact_yang_warm（对立，仍报）
 *     丹参 waive=true → transparent_therapy_herb_support（非对立，塌回覆盖率码，在本集合内）
 * 也就是说：**能走到这里的 unsupported_high_impact 码，方向必然是对立的**。
 * 把它加进来等于让「附子进热证」这类方向相反的药进入已受理的剂量级处方。
 *
 * 我曾按「两处必须同集」把本函数改成直接问 policy 要批注——那是个 fail-open 回归，
 * 由对抗核验实测拦下。此处留痕：policy 的批注是**给医生写文案**用的（作用在 waive=false
 * 重算出来的码上），本函数是**复验时放不放行**，两者输入总体本就不同。
 * 「同集」在这一对上不是不变量，不要再往这个方向改。
 */
