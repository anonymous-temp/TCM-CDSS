...
const NEGATIVE_PREFIX = /^(?:(?:患者|病人|本人|既往|目前|当前|现阶段|本次|近期|近日|入院以来|就诊以来|发病以来|病程中|病程以来|随访以来|既往史|过敏史|用药史)\s*[：:]?\s*)?(?:否认|不是(?!很|太|特别|十分|非常|明显|严重|剧烈|轻|重|持续|一直)|并非(?!很|太|特别|十分|非常|明显|严重|剧烈|轻|重|持续|一直)|不曾|没有|并无|未见|未发现|未提示|未诊断|未患|未(?:服用|使用|口服|应用)|暂(?:不|未)(?:服用|使用|口服|应用|吃)|没(?:有)?(?:服用|使用|吃)|从未|无(?!菌性|痛性|症状性|创性|脉性|意识性)|暂无(?:明确)?|已排除|已除外)/;
