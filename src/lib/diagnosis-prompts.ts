...
**辨证按「证素先行」的顺序进行——先识证素、再组证名，不是先想证名再倒推证素。**
这是本系统辨证链的固定顺序（证素辨证：任何证都是病位要素与病性要素的组合）：
① 先从本例四诊事实中逐条识别**病位证素**（心、肝、脾、肺、肾、胃、胆、大肠、小肠、膀胱、
胞宫、经络、脑窍、肌肤、筋骨等）——每条都要指得到具体的症状、舌脉或病史依据；
② 再逐条识别**病性证素**（气虚、血虚、阴虚、阳虚、气滞、血瘀、痰、湿、水饮、寒、热、火、
风、燥、毒、食积等），同样逐条指到依据；
③ 最后由已识别的证素**组合**出规范证名写入 overview.primarySyndrome（病位=心+脾、
病性=气虚+血虚 ⇒「心脾两虚证」；病位=肝+胃、病性=热 ⇒「肝胃郁热证」），
并把同一批证素同步写入 locationDifferentiation.items 与 natureDifferentiation.items。
证名与两个 items 必须互相对得上：证名里出现的病位病性，两个 items 里必须有；
items 里已确认的核心证素，也不应在证名里凭空消失。四诊资料只支持病性而定不到脏腑病位时，
可只给病性证素并让病位保持 bounded/unresolved，但不得因此跳过①②直接写一个笼统证名。
按证素组合出的规范证名比笼统描述更可用：它决定了后续能否检索到对证的经典方。
