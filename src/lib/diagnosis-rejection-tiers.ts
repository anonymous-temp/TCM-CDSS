...
const T2_M04_PATTERNS: readonly RegExp[] = [
  ...
  // 逐方向治法覆盖（甲方 2026-08-13 寒凝血瘀痛经：治法写温经散寒、方中无温药）。
...
export function qualityAnnotationCopy(reason: string): string | undefined {
  ...
  if (prescribe) {
    ...
    return tier === "T2"
      ? "本次候选已完成确定性核查，具体用量仍需结合药味表中的历史参考来源由医生确认；部分调护建议、加减说明或方义描述仍不完整，请结合本次病历核对。"
      : "本次候选已完成确定性核查，具体用量仍需由医生确认；医生可见正文与结构化结果之间存在个别展示差异，用药内容以药味清单为准。";
  }
  return tier === "T2"
