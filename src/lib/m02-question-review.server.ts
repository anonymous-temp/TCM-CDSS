...
import { createTextModelClient, getPrimaryTextModelConfig, isApprovedTextModel, textModelConfigForModel, textModelRequestTuning } from "./text-model";
...
// 复核挂在 M02 流的尾部，医生要等它跑完才看得到问题。线上实测（qwen3.8-flash）p50 5.0s、
// p90 6.2s，22 次里 1 次撞满 15s。放到 20s 并改成环境变量可调，免得调一次预算重建一次镜像。
const M02_QUESTION_REVIEW_TIMEOUT_MS = (() => {
  const value = Number(process.env.M02_QUESTION_REVIEW_TIMEOUT_MS || 20_000);
...
function defaultModelCall(): ReviewModelCall | null {
  // M02 is generated by the fast primary model. Semantic known-answer / exclusivity checks may use a
  // separate approved model via PRIMARY_REVIEW_MODEL so the generator does not grade its own wording.
  // (PRIMARY_CLINICAL_REVIEW_MODEL belonged to the removed M03/M04 clinical reviewer, 2026-09-16.)
  //
  // 端点必须跟着**复核模型**走，不能沿用主 provider：旧写法用主 provider 的端点与密钥发出
  // PRIMARY_REVIEW_MODEL，跨厂商取值会 100% 上游失败，而两次抽样全失败是静默退回确定性初判，
  // 与「模型认为无需改动」无法区分。textModelConfigForModel 在目标家族没配齐时 fail-closed。
  const reviewModel = process.env.PRIMARY_REVIEW_MODEL?.trim();
  const config = reviewModel ? textModelConfigForModel(reviewModel) : getPrimaryTextModelConfig();
  if (!config.configured || !isApprovedTextModel(config.model)) return null;
  const client = createTextModelClient(config);
  return async (prompt, signal) => {
    const completion = await observeModelTask({ task: "m02_question_review", stage: "question", model: config.model }, () => client.chat.completions.create({
      model: config.model,
      temperature: 0,
      max_tokens: 900,
...
export async function reviewM02QuestionPlan(
  ...
  const timeout = setTimeout(() => controller.abort(new Error("m02_question_review_timeout")), M02_QUESTION_REVIEW_TIMEOUT_MS);
  try {
    const prompt = [
