...
export function buildDeterministicFormulaReferenceFallback(
  ...
  return [
    ...
    "## 当前结论",
    recallBased
      ? "本轮模型未能形成可核验的剂量级候选处方（输出截断或未通过结构化合同）。M03 本次未锁定经典方；以下为按已签名证候/病机**确定性检索**到的标准方剂参考（未经本例辨证锁定，证据等级低于锁定方），组成与药典剂量区间均来自标准方剂资料，供医生参考判断。"
      : "本轮模型未能形成可核验的剂量级候选处方（输出截断或未通过结构化合同）。以下为 M03 已锁定方剂的**标准基准组成与药典剂量区间**——全部来自标准方剂资料的确定性数据，不含模型生成内容，供医生直接参考定量。",
    "",
    ...(syndrome || method
