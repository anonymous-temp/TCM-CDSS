...
- **Per-stage models (see `.env.example`; owner decision 2026-09-19: two tiers only, `qwen3.8-flash` + `qwen3.8-max`).** The provider is `AI_TEXT_PROVIDER=bailian-qwen` (DashScope compatible-mode). Production sets every model variable explicitly in the release override (`artifacts/tencent-release-20260919-qwen38/release-ops/production.override.yml` on the host), so the runtime env's leftover values do not leak in. History: 2026-09-11→09-19 production ran every text phase on `deepseek-flash`, whose `json_object` mode does not enforce the schema — the M03 Western half came back as misnested JSON on every case and the page showed the placeholder diagnosis. Both remaining tiers enforce strict JSON Schema; that is the reason for the switch back:

  | Phase | Model | Why |
  |---|---|---|
  | M02 追问 + all small tasks (terminology, rerank, formula recall, polarity assist, M05 authoring, M02 interpret/review, medicine planner) | `BAILIAN_QWEN_MODEL` / `CONTROLLED_TERMINOLOGY_MODEL` = `qwen3.8-flash` | M02 has no `PRIMARY_QUESTION_MODEL`; it follows the provider base model. Measured slower than the former `qwen3.7-flash` on small calls (facts extract ~3–4 s vs 1.2 s), so the 6 s budgets of rerank/recall/polarity have less headroom — they fall back to deterministic results on timeout |
  | **M03 first pass** (both parallel halves) | **`qwen3.8-flash`** | Thinking disabled, strict JSON Schema. The provider schema also pins 8 clinically mandatory arrays to `minItems:1` (`requireGeneratedM03Content` in `model-response-format.ts`): without it strict decoding returned 主证候依据/病名与证候鉴别/病位/病性/子治法/待核实信息 empty in most runs while marking them `resolved` (paired replay 2026-09-19). `qwen3.7-flash` is **not** a rollback — it has no strict mode |
  | M04 first pass | `qwen3.8-flash` | Owner dropped `qwen3.7-plus` (measured ~6 s faster on M04) to keep two tiers |
  | M03/M04 repair rounds, M04 transport fallback | `qwen3.8-max` | Rare paths; quality decides the outcome |
  ...
  | Clinical facts | `qwen3.8-max` for all three phases (extract / review / adjudicate) | Chosen from full golden-baseline runs (222 cases) on 2026-09-19. Flash extraction often misses spoken-language risks: `advisory-low-temp-35-5` and `negated-dizziness-then-severe-headache` each passed 1/3 fresh-server samples. A flash reviewer was slower than max (p50 4.1 s vs 3.1 s), timed out 14 times and produced invalid reviews, which failed 21 assertions. All-max left one conservative failure (`advisory-respiration-25` asks to recheck before dosing). Same model in every phase ⇒ `independentReview`/`independentAdjudication` are false ⇒ the layer can add risk but never downgrade a grounded first-pass finding — the same semantics as the DeepSeek period |

  DeepSeek (`OPENAI_MODEL=deepseek-v4-flash` + `OPENAI_BASE_URL`) stays configured as the **wholesale rollback tier** — set `AI_TEXT_PROVIDER=openai-compatible` to switch the whole chain back (in production: re-run compose with the 2026-09-11 deepseek-flash override). It is not active in production since 2026-09-19. `isApprovedTextModel` = qwen\* ∪ deepseek\*. Clinical-facts review/adjudication phases remain separate requests; safety phases may add risk but can never erase or downgrade grounded risk. `reasoning_effort` / `thinking_enabled` remain per-stage controls, both defaulting OFF (reasoning-only streams are treated as errors).
