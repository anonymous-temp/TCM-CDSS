...
function runFrontendContractChecks() {
  ...
  assert(envExample.includes("AI_TEXT_PROVIDER=bailian-qwen") && envExample.includes("BAILIAN_QWEN_MODEL=qwen3.7-plus") && envExample.includes("PRIMARY_DIAGNOSE_MODEL=qwen3.7-flash") && envExample.includes("PRIMARY_PRESCRIBE_MODEL=qwen3.7-plus") && envExample.includes("PRIMARY_CLINICAL_REVIEW_PROVIDER=primary") && envExample.includes("PRIMARY_CLINICAL_REVIEW_MODEL=qwen3.8-max") && envExample.includes("PRIMARY_CLINICAL_REVIEW_REASONING_EFFORT=low") && envExample.includes("M04_ORCHESTRATION_DEADLINE_MS=180000") && composeFile.includes("AI_TEXT_PROVIDER:-bailian-qwen") && composeFile.includes("BAILIAN_QWEN_MODEL:-qwen3.7-plus") && composeFile.includes("PRIMARY_DIAGNOSE_MODEL:-qwen3.7-flash") && composeFile.includes("PRIMARY_PRESCRIBE_MODEL:-qwen3.7-plus") && composeFile.includes("PRIMARY_CLINICAL_REVIEW_PROVIDER:-primary") && composeFile.includes("PRIMARY_CLINICAL_REVIEW_MODEL:-qwen3.8-max") && composeFile.includes("PRIMARY_CLINICAL_REVIEW_TIMEOUT_MS:-40000") && composeFile.includes("M04_ORCHESTRATION_DEADLINE_MS:-180000"), "deploy: measured Qwen 3.7 Flash remains the M03 default; M04 stays on Plus with Max review and a 180s orchestration cap", `${envExample}\n${composeFile}`);
