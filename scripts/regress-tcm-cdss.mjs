...
async function runFrontendContractChecks() {
  ...
  assert(["classical-formula", "warm-disease", "warm-tonify", "support-yang", "nourish-yin-danxi"].every((code) => lineageSource.includes(code)), "lineage: every published school is covered by the question strategy map", lineageSource);
  ...
  assert(diagnoseRoute.includes("降低相应结论置信度") && diagnoseRoute.includes("【就诊目标待确认】") && !diagnoseRoute.includes("historicalOnlyEncounter") && diagnoseRoute.includes("hasValidClinicalFactsAttestation") && diagnoseRoute.includes("sanitizeUngroundedRedFlagNegations(content, safeState)"), "backend: limited-information M03 gets a non-blocking uncertainty boundary, attested-unclear visit scope (historical-only is context, never a downgrade), and grounded negative-history guard", diagnoseRoute);
  assert(
  ...
  assert(diagnosisTypesSource.includes("level: determineCompletenessLevel(scores)") && !/const level = input\.level/.test(diagnosisTypesSource), "types: deserialized completeness level is recomputed from four dimensions", sourceBetween(diagnosisTypesSource, "function normalizeCompleteness", "function normalizeSafetyGate"));
  ...
  assert(envExample.includes("AI_TEXT_PROVIDER=bailian-qwen") && envExample.includes("BAILIAN_QWEN_MODEL=qwen3.8-flash") && envExample.includes("PRIMARY_QUESTION_MODEL=deepseek-flash") && envExample.includes("PRIMARY_DIAGNOSE_MODEL=deepseek-flash") && envExample.includes("PRIMARY_PRESCRIBE_MODEL=deepseek-flash") && envExample.includes("PRIMARY_STRUCTURED_FALLBACK_MODEL=qwen3.8-flash") && envExample.includes("M04_ORCHESTRATION_DEADLINE_MS=180000") && composeFile.includes("AI_TEXT_PROVIDER:-bailian-qwen") && composeFile.includes("BAILIAN_QWEN_MODEL:-qwen3.8-flash") && composeFile.includes("PRIMARY_QUESTION_MODEL:-deepseek-flash") && composeFile.includes("PRIMARY_DIAGNOSE_MODEL:-deepseek-flash") && composeFile.includes("PRIMARY_PRESCRIBE_MODEL:-deepseek-flash") && composeFile.includes("PRIMARY_STRUCTURED_FALLBACK_MODEL:-qwen3.8-flash") && composeFile.includes("M04_ORCHESTRATION_DEADLINE_MS:-180000"), "deploy: DeepSeek Flash is the M02/M03/M04 first-pass default behind a Qwen 3.8 Flash strict-schema fallback (since 2026-09-24; Qwen flash+max tiers since 2026-09-19) with a 180s M04 orchestration cap (the model reviewer was removed 2026-09-16)", `${envExample}\n${composeFile}`);
  ...
  assert(composeFile.includes("IMAGE_TAG:?set immutable IMAGE_TAG") && composeFile.includes("CDSS_API_TOKEN:?set CDSS_API_TOKEN") && composeFile.includes("CASE_SNAPSHOT_ENCRYPTION_KEY:?set CASE_SNAPSHOT_ENCRYPTION_KEY") && composeFile.includes("REASONING_CONTRACT_SIGNING_KEY:?set REASONING_CONTRACT_SIGNING_KEY") && composeFile.includes("healthcheck:") && composeFile.includes("strictReady!==true") && composeFile.includes("x-cdss-api-token") && /CDSS_HEALTHCHECK_LIVE_PROBES==='true'\?'\?strict=1':''/.test(composeFile) && composeFile.includes("CDSS_HEALTHCHECK_LIVE_PROBES: ${CDSS_HEALTHCHECK_LIVE_PROBES:-false}"), "deploy: immutable image tag, access token, snapshot key, reasoning signature key and an authenticated strictReady healthcheck are mandatory; live upstream probes are an explicit opt-in (paused 2026-09-20: ~1000 model calls/day)", composeFile);
