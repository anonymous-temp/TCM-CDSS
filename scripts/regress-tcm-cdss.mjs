...
async function runFrontendContractChecks() {
  ...
  assert(envExample.includes("AI_TEXT_PROVIDER=bailian-qwen") && envExample.includes("BAILIAN_QWEN_MODEL=qwen3.8-flash") && envExample.includes("PRIMARY_DIAGNOSE_MODEL=qwen3.8-flash") && envExample.includes("PRIMARY_PRESCRIBE_MODEL=qwen3.8-flash") && envExample.includes("M04_ORCHESTRATION_DEADLINE_MS=180000") && composeFile.includes("AI_TEXT_PROVIDER:-bailian-qwen") && composeFile.includes("BAILIAN_QWEN_MODEL:-qwen3.8-flash") && composeFile.includes("PRIMARY_DIAGNOSE_MODEL:-qwen3.8-flash") && composeFile.includes("PRIMARY_PRESCRIBE_MODEL:-qwen3.8-flash") && composeFile.includes("M04_ORCHESTRATION_DEADLINE_MS:-180000"), "deploy: Qwen 3.8 Flash is the M02/M03/M04 first-pass default (two tiers flash+max since 2026-09-19) with a 180s M04 orchestration cap (the model reviewer was removed 2026-09-16)", `${envExample}\n${composeFile}`);
