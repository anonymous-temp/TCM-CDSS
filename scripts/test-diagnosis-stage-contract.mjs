...
const assessWithoutLabs = buildTcmKnowledgeContext({ patient: {}, chiefComplaint: "失眠", conversation: [] }, "assess");
assert.doesNotMatch(assessWithoutLabs, /实验室\/检查阈值规则/, "零命中时不得向 M05 注入任意前6条检验阈值");
const negatedPatentRisk = buildTcmKnowledgeContext({ patient: {}, chiefComplaint: "否认服用藿香正气水", conversation: [] }, "assess");
assert.doesNotMatch(negatedPatentRisk, /命中中成药重点风险/, "明确否认的中成药不得反向召回阳性风险");
const affirmedPatentRisk = buildTcmKnowledgeContext({ patient: {}, chiefComplaint: "正在服用藿香正气水", conversation: [] }, "assess");
assert.match(affirmedPatentRisk, /命中中成药重点风险/, "当前肯定用药仍须召回对应风险");
...
dirtyPrecautionsProposalInput.nonPharma.precautions = [
  ...
  "患者正服苯磺酸氨氯地平片，中药汤剂宜与其间隔至少1-2小时服用。", // 代词式药物间隔同样丢弃
...
assert.equal(
  keptPrecautions.some((item) => /氨氯地平|间隔2小时|间隔至少1-2小时|影响吸收/.test(item)),
  false,
