...
// 三之前补一类（2026-08-13 线上实测）：并列标准证候被「病名前缀包含式匹配」误伤。
// 「心阳不振，水气凌心」两段都是 GB/T 16751.2 收录证候，但前缀「心阳不振，水气」尾部
// 撞上病名「水气」，剩下的「凌心」解析不出证候，整串被误判成「病名+病机」形态——
// 修复轮空转、方名锁定被搁置、M04 只能出药典区间参考页。判据现在先查并列段。
ok("心阳不振，水气凌心", "并列标准证候：前缀尾撞病名「水气」的线上原串");
ok("心阳不振,水气凌心", "同上，半角逗号变体");
