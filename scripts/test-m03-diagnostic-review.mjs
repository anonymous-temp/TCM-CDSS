...
const bowelReviewPrompt = buildM03DiagnosticReviewPrompt("主诉：便秘。年龄：58岁。现病史：近期新发排便困难，既往筛查史未知。", {});
assert.doesNotMatch(reviewPrompt, /中老年新发或进行性排便习惯改变做患者特异的报警征象审计/, "unrelated sleep cases need no bowel-specific paragraph");
assert.match(bowelReviewPrompt, /中老年新发或进行性排便习惯改变做患者特异的报警征象审计/);
assert.match(bowelReviewPrompt, /不能再用‘若年龄>40岁\/50岁’这种未实例化的假设句/);
assert.match(bowelReviewPrompt, /本例已满足的年龄与病程条件/);
