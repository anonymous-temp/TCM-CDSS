...
const scripts = [
  ...
  // 第二个类目是确定性层自己的：「既往体健，这次胸痛伴大汗」「否认高血压病史，血压190/125mmHg」
  // 「3天前开始发烧，伴一次抽搐」在安全门上 ready、给剂量——既往锚点越过逗号、当前锚点闭集不全。
  // 整类在这里用兄弟句法量（背景句+急症 必须与单说急症同判），反例钉住「…病史，最高血压…」
  // 「…人流史，术后流血…」这类既往细节不被抬成当前急症。
  "test:open-language-classes",
  // 2026-09-26 开方前处置去向（开放判断，类目表之外的必须排除情况）：解析闭集、原文接地（否认/已缓解不算、
  // 疑似措辞算）、门禁映射（急诊→红旗、宜先专科→扣剂量、还差一问→只提示）、签名覆盖（删掉即失效）。
  // 起因：车祸伤膝关节畸形、无痛性黄疸+胰头占位在类目表之外，类目式抽取一条不报。
  "test:clinical-disposition",
  ...
  "test:prompt-context-experience",
  ...
  "test:inventory-customer-migration",
