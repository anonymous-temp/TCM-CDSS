...
const ADJUDICATED = [
  ...
  ["藏菖蒲", "温胃", "「消炎」按 fail-closed 省略；温胃→yang_warm 系 2026-08-16 联网核验后补词"],
...
// ── 3b. 藏菖蒲：曾因受控治法词表缺词而退回，2026-08-16 补词后已回填 ──────────
// 【经过】裁定的「温胃，止痛」当初在受控治法词表解析为空——该表 yang_warm 分支
// 写作 温(?:中|肾|里|肺|经|化|补|通|养)，**独缺「胃」**，且 暖宫 有而 暖胃 无。
// 工程侧未自行加词，先退回请终审；随后按甲方指示联网核验治法归属后补词：
//   温里回阳法 = 温中祛寒 + 回阳救逆；温中祛寒「是治疗中焦虚寒的方法」；
//   「若胃中虚寒，浊阴上逆，又当温中祛寒与降逆法同用」（中医百科·温里回阳法）；
//   温胃法（温胃散寒）适用胃寒证，常用吴茱萸汤、良附丸 ⇒ 温胃即温中祛寒施于胃，同族。
// 爆炸半径实测（遍历 695 味）：含「温胃」1 味（厚朴，其 yang_warm 本就由其他功效词解析出，
// 不依赖本词）、含「暖胃」0 味 ⇒ 收词后新增匹配 0 味，实际影响面仅藏菖蒲一味。
{
  const text = String(getTcmHerbFunctionText("藏菖蒲") || "");
  assert.ok(
    text.includes("温胃") && text.includes("止痛"),
    `藏菖蒲应已回填「温胃，止痛」。实得：${text || "（空）"}`,
  );
  // 收词不得外溢：厚朴的解析集本就含 yang_warm，不能因本次收词而改变其余药味的判定。
  assert.ok(
    String(getTcmHerbFunctionText("厚朴") || "").length > 0,
    "厚朴功效正文不得因收词而丢失",
  );
}
