...
- **模型接入**：`openai` SDK，OpenAI 兼容协议。生产 `AI_TEXT_PROVIDER=bailian-qwen`（百炼 compatible-mode），文本相位自 2026-09-19 起**只用两档**（owner 裁定）：`qwen3.8-flash` 负责药品规划/M02 解释，并作为 M02 出题、M03 两个半程、M04 首轮的严格兜底（**2026-09-24 起这三处首轮改跑 `deepseek-flash`**，见下文「分阶段模型配置」；**2026-09-20 起小任务跨厂商跑 `deepseek-flash`**：术语归一/证候重排/方名召回/否定辅助/M05 作文由 `CONTROLLED_TERMINOLOGY_MODEL` 驱动，M02 出题复核由 `PRIMARY_REVIEW_MODEL` 驱动——这些任务只用 json_object、从不需要严格 schema，而 DeepSeek 出字快 2–5 倍：同机同时段真实提示词 A/B 实测术语 6673→1354ms、重排 5549→984ms、M05 作文 9587→4163ms、M02 复核约 5000→1150ms，临床字段无差异。端点与密钥由 `text-model.textModelConfigForModel` **按模型家族**解析，DeepSeek 模型绝不会打到 dashscope 端点；目标家族没配齐时 fail-closed，各任务退回确定性结果。回滚改回 `qwen3.8-flash` 即可，不需改代码），`qwen3.8-max` 负责 M03/M04 修复轮、M04 传输兜底与临床事实抽取。**2026-09-20 起事实层复核相位缺省关闭**（`CDSS_CLINICAL_FACTS_REVIEW=true` 才开）：黄金基线 198 例重放，复核对安全门输入的净差异为 0 例，每次冷启动多约 3.6s、每个病人付两次；关闭后结果标 `single_pass`，由 `clinical-facts.clinicalFactsReviewSettled` 这一个谓词统一判定为完成态（门禁急症升级、筛查完成、签名、缓存、red-flags 路由共用），否则每一例都会被判「复核未完成」扣掉剂量。「仅既往/稳定」这类下调判断仍要求两次一致，复核关闭时保持 unreviewed、按活动性就诊处理（保守方向）。**同日起 M04 修复轮只重写处方主体**：主原因与同批每个 T1 问题都落在 candidate 内时，只让模型输出 `{candidate}`（`m04_candidate_patch` schema），服务端拼回上一版原始提案，中成药/西药、加减、非药物调护逐字保留；有 candidate 以外的 T1 问题才整份重写。术语归一与 M02 出题复核都改为只调一次（原为同一提示词 temperature 0 连发两遍取一致）。事实层按黄金基线实测定档：flash 抽取常漏标口语风险（低体温 35.5℃、「否认头晕，剧烈头痛」冷启动各 3 次只过 1 次），flash 复核更慢且超时/不合格输出多（整套基线 21 条失败）；全 max 时整套基线只剩 1 条偏保守的失败。三相位同一模型 ⇒ 两个「独立」判定为假 ⇒ 只能加风险、不能下调首轮风险（与 DeepSeek 期同一语义）。两档都执行严格 JSON Schema；M03 下发 schema 另把 8 个临床必有内容的数组钉成 minItems:1（`model-response-format.ts` 的 `requireGeneratedM03Content`），否则严格解码会把主证候依据、鉴别、病位病性、子治法、待核实信息写成空数组。`qwen3.7-flash` 没有严格模式，不能再当回滚。9/11–9/19 生产曾整链跑 `deepseek-flash`（json_object 不执行 schema，西医半每例括号错位），DeepSeek 仍保留为整体回退档（改 `AI_TEXT_PROVIDER=openai-compatible`），生产未启用。GLM 视觉默认开启且仅用于舌象图片
- **校验**：zod 4
...
- **分阶段模型配置**（见 `.env.example`，2026-08-27 已与生产 env + 30h `model_usage` 遥测核对）：分档矩阵见上；`reasoning_effort` / `thinking_enabled` 同为分阶段环境变量，缺省一律关（仅返回推理过程的流视为错误）。**不存在 `PRIMARY_COLLECT_MODEL`**：M01 文本路径根本不调模型（无舌象图时 collect 路由直接返回确定性 NDJSON）。**`PRIMARY_QUESTION_MODEL` 自 2026-09-24 起存在**，只管 M02 出题这一个调用点；未设时 M02 跟随供应商基础模型（`BAILIAN_QWEN_MODEL`）。
  - **首轮生成跑 DeepSeek、Qwen strict 兜底（2026-09-24，提速第三批）**：M02 出题、M03 两半、M04 首轮 = `deepseek-flash`；`PRIMARY_STRUCTURED_FALLBACK_MODEL` = `qwen3.8-flash`。生产机直连配对重放本周真实提示词：西医半 24→8s、中医半 29→10s、M04 27→11s、M02 15→4s。DeepSeek 只有 json_object，三件事缺一不可：① M03 两半的系统消息附严格供应商的同一份 schema（西医半合规 0/18→18/18、中医半 12/18→18/18；**M04 不附**，附了反而 14/14→11/14），中医半另重申提示词已有的三轴 resolution 定档规则（DeepSeek 否则 16/18 三轴全写 bounded）；② 服务端用同一份 schema 逐项校验（`checkNonStrictStructuredContent`：只补末尾缺失括号、超长数组截到上限、可空字段空串=省略、服务端自有键容忍，其余一律算违规）；③ 违规、连不上、任何非 2xx（含 402）或流中途断 → 严格兜底模型对同一份提示词重生成（非流式），质量下限即改动前。端点处处按模型家族解析；健康检查逐阶段报 `*_model_not_configured`。事实层三相位**不能**换 DeepSeek（9/11 实测 35.5℃ 放行剂量）。回滚：三个阶段变量改回 `qwen3.8-flash`，无需重建。
