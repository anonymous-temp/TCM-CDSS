...
# 构建溯源:证明线上镜像 == 测过的源码(甲方评测第 10 条的元缺陷)。
ARG CDSS_BUILD_COMMIT="unknown"
ARG CDSS_BUILD_SOURCE_DIGEST="unknown"
ARG CDSS_BUILD_TIMESTAMP="unknown"
ENV NEXT_PUBLIC_BASE_PATH=$NEXT_PUBLIC_BASE_PATH
ENV NEXT_PUBLIC_ENABLE_BROWSER_CASE_PERSISTENCE=$NEXT_PUBLIC_ENABLE_BROWSER_CASE_PERSISTENCE
ENV NEXT_DEPLOYMENT_ID=$NEXT_DEPLOYMENT_ID
ENV CDSS_BUILD_COMMIT=$CDSS_BUILD_COMMIT
ENV CDSS_BUILD_SOURCE_DIGEST=$CDSS_BUILD_SOURCE_DIGEST
ENV CDSS_BUILD_TIMESTAMP=$CDSS_BUILD_TIMESTAMP
